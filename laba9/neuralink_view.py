# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'neuralink_view.ui'
#
# Created by: PyQt5 UI code generator 5.15.3
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_form_neuralink(object):
    def setupUi(self, form_neuralink):
        form_neuralink.setObjectName("form_neuralink")
        form_neuralink.resize(200, 350)
        form_neuralink.setMinimumSize(QtCore.QSize(200, 350))
        form_neuralink.setMaximumSize(QtCore.QSize(200, 350))
        form_neuralink.setInputMethodHints(QtCore.Qt.ImhNone)
        self.pb_predict = QtWidgets.QPushButton(form_neuralink)
        self.pb_predict.setGeometry(QtCore.QRect(100, 280, 85, 32))
        self.pb_predict.setObjectName("pb_predict")
        self.lb_number = QtWidgets.QLabel(form_neuralink)
        self.lb_number.setGeometry(QtCore.QRect(20, 15, 71, 16))
        self.lb_number.setObjectName("lb_number")
        self.gv_digit = QtWidgets.QGraphicsView(form_neuralink)
        self.gv_digit.setGeometry(QtCore.QRect(10, 35, 180, 180))
        self.gv_digit.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.gv_digit.setLineWidth(0)
        self.gv_digit.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.gv_digit.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        self.gv_digit.setObjectName("gv_digit")
        self.pb_train = QtWidgets.QPushButton(form_neuralink)
        self.pb_train.setGeometry(QtCore.QRect(15, 280, 85, 32))
        self.pb_train.setObjectName("pb_train")
        self.pb_clear = QtWidgets.QPushButton(form_neuralink)
        self.pb_clear.setGeometry(QtCore.QRect(45, 310, 110, 32))
        self.pb_clear.setObjectName("pb_clear")
        self.le_result = QtWidgets.QLineEdit(form_neuralink)
        self.le_result.setGeometry(QtCore.QRect(10, 250, 181, 21))
        self.le_result.setReadOnly(True)
        self.le_result.setObjectName("le_result")
        self.lb_result = QtWidgets.QLabel(form_neuralink)
        self.lb_result.setGeometry(QtCore.QRect(20, 230, 71, 16))
        self.lb_result.setObjectName("lb_result")

        self.retranslateUi(form_neuralink)
        QtCore.QMetaObject.connectSlotsByName(form_neuralink)

    def retranslateUi(self, form_neuralink):
        _translate = QtCore.QCoreApplication.translate
        form_neuralink.setWindowTitle(_translate("form_neuralink", "Neuralink"))
        self.pb_predict.setText(_translate("form_neuralink", "Predict"))
        self.lb_number.setText(_translate("form_neuralink", "Write digit:"))
        self.pb_train.setText(_translate("form_neuralink", "Train"))
        self.pb_clear.setText(_translate("form_neuralink", "Clear"))
        self.lb_result.setText(_translate("form_neuralink", "Prediction:"))
